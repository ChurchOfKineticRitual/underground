...
const LINE_ID = 'bakerloo';
const LINE_DEPTH_M = 25; // heuristic fallback
...
// load anchors (may be empty for Bakerloo)
let anchors = new Map();
try {
...
await fs.mkdir('public/data/bakerloo', { recursive: true });
await fs.writeFile('public/data/bakerloo/shafts.json', JSON.stringify({
  line: LINE_ID,
  origin: ORIGIN,
  verticalScale,
  groundY,
