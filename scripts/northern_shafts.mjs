...
const LINE_DEPTH_M = 26; // heuristic fallback (Northern has deep sections)
...
// load anchors (may be empty for Northern)
let anchors = new Map();
try {
...
await fs.mkdir('public/data/northern', { recursive: true });
await fs.writeFile('public/data/northern/shafts.json', JSON.stringify({
  line: LINE_ID,
  origin: ORIGIN,
  verticalScale,
  groundY,
