...
const LINE_ID = 'district';
const LINE_DEPTH_M = 12; // heuristic fallback (District is cut-and-cover, shallower)
...
// load anchors (may be empty for District)
let anchors = new Map();
try {
...
await fs.mkdir('public/data/district', { recursive: true });
await fs.writeFile('public/data/district/shafts.json', JSON.stringify({
  line: LINE_ID,
  origin: ORIGIN,
  verticalScale,
  groundY,
