...
const LINE_ID = 'circle';
const LINE_DEPTH_M = 12; // heuristic fallback (Circle is cut-and-cover, shallower)
...
// load anchors (may be empty for Circle)
let anchors = new Map();
try {
...
await fs.mkdir('public/data/circle', { recursive: true });
await fs.writeFile('public/data/circle/shafts.json', JSON.stringify({
  line: LINE_ID,
  origin: ORIGIN,
  verticalScale,
  groundY,
