...
const LINE_ID = 'dlr';
const LINE_DEPTH_M = 8; // heuristic fallback (DLR is mostly surface/elevated, shallow where underground)
...
// load anchors (may be empty for DLR)
let anchors = new Map();
try {
...
await fs.mkdir('public/data/dlr', { recursive: true });
await fs.writeFile('public/data/dlr/shafts.json', JSON.stringify({
  line: LINE_ID,
  origin: ORIGIN,
  verticalScale,
  groundY,
