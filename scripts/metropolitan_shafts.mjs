...
const LINE_ID = 'metropolitan';
const LINE_DEPTH_M = 10; // heuristic fallback (Metropolitan is cut-and-cover, shallow)
...
// load anchors (may be empty for Metropolitan)
let anchors = new Map();
try {
...
await fs.mkdir('public/data/metropolitan', { recursive: true });
await fs.writeFile('public/data/metropolitan/shafts.json', JSON.stringify({
  line: LINE_ID,
  origin: ORIGIN,
  verticalScale,
  groundY,
